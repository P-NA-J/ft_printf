...
/*   Updated: 2019/04/08 15:37:05 by pauljull         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		convert_option(const char *restrict format, int *flag, int *i_ptr)
{
	...
	|| format[i] == '#' || format[i] == ' ')
	{
		if (format[i] == '#' && !bit_value(*flag, 15))
			*flag += (1 << 15);
		if (format[i] == '0' && !bit_value(*flag, 16))
			*flag += (1 << 16);
		if (format[i] == ' ' && !bit_value(*flag, 17))
			*flag += (1 << 17);
		if (format[i] == '+' && !bit_value(*flag, 18))
			*flag += (1 << 18);
		if (format[i] == '-' && !bit_value(*flag, 19))
			*flag += (1 << 19);
		i += 1;
	}
	*i_ptr = i;
...
void		correct_parse(t_list **list_ptr)
{
	t_list *list;
	...
	while (list)
	{
		if (bit_value(list->flag, 16))
			if (list->precision || bit_value(list->flag, 19))
				list->flag -= (1 << 16);
		if (bit_value(list->flag, 17))
			if (bit_value(list->flag, 18))
				list->flag -= (1 << 17);
		if (bit_value(list->flag, 11))
